...
class Entity(NodePath, metaclass=PostInitCaller):
    ...
    default_values = {
        # 'parent':scene,
        'name': 'entity', 'enabled': True, 'eternal': False, 'position': Vec3(0, 0, 0), 'rotation': Vec3(0, 0, 0),
        'scale': Vec3(1, 1, 1), 'model': None, 'origin': Vec3(0, 0, 0),
    ...
    def __init__(self, add_to_scene_entities=True, enabled=True, **kwargs):
        ...
        if application.trace_entity_definition and add_to_scene_entities or (
                not _Ursina_instance and _warn_if_ursina_not_instantiated and add_to_scene_entities):
        ...
        if not _Ursina_instance and _warn_if_ursina_not_instantiated and add_to_scene_entities:
            print_warning(
                'Tried to instantiate Entity before Ursina. Please create an instance of Ursina first (app = Ursina())',
        ...
        for key in (
        'model', 'origin', 'origin_x', 'origin_y', 'origin_z', 'collider', 'shader', 'texture', 'texture_scale',
        'texture_offset'):
            if key in kwargs:
        ...
        for method in every.decorated_methods:
            if get_class_name(method._func) == self.types[0]:
                self.animations.append(
                    Sequence(Func(method, self), Wait(method._every.interval), loop=True, started=True, entity=self))
    ...
    def render_queue_setter(self,
                            value):  # for custom sorting in case of conflict. To sort things in 2d, set .z instead of using this.
    ...
    def parent_setter(self, value):
        if hasattr(self, '_parent') and self._parent and hasattr(self._parent,
                                                                 '_children') and self in self._parent._children:
    ...
    def texture_scale_getter(
            self):  # (A tuple of x and y) The amount of times the texture is repeated. For example, texture_scale=(2,2), means that the texture is fit four times into each face of a cube.
    ...
    def tileset_size_getter(
            self):  # if the texture is a tileset, say how many tiles there are so it only use one tile of the texture, e.g. tileset_size=[8,4]
    ...
    def bounds(self):
        _bounds = self.model_bounds
        return Bounds(start=_bounds.start * self.scale, end=_bounds.end * self.scale, center=_bounds.center,
                      size=_bounds.size * self.scale)

    def get_position(self,
                     relative_to=scene):  # get position relative to on other Entity. In most cases, use .position instead.
        return self.getPos(relative_to)

    def set_position(self, value,
                     relative_to=scene):  # set position relative to on other Entity. In most cases, use .position instead.
    ...
    def animate(self, name, value, duration=.1, delay=0, curve=curve.in_expo, loop=False, resolution=None,
        ...
        if delay:
            ...
            return invoke(self.animate, name, value, duration=duration, curve=curve, loop=loop, resolution=resolution,
                          time_step=time_step, auto_destroy=auto_destroy, delay=delay, unscaled=unscaled)

        animator_name = name + '_animator'
        # print('start animating value:', name, animator_name )
        if interrupt and hasattr(self, animator_name):
            getattr(getattr(self, animator_name),
                    interrupt)()  # call kill() or finish() depending on what the interrupt value is.
    ...
    def shake(self, duration=.2, magnitude=1, speed=.05, direction=(1, 1), delay=0, attr_name='position',
              interrupt='finish', unscaled=False):
        import random
    ...
    def fade_out(self, value=0, duration=.5, unscaled=False, **kwargs):
        return self.animate('color', Vec4(self.color[0], self.color[1], self.color[2], value), duration=duration,
                            unscaled=unscaled, **kwargs)

    def fade_in(self, value=1, duration=.5, unscaled=False, **kwargs):
        return self.animate('color', Vec4(self.color[0], self.color[1], self.color[2], value), duration=duration,
                            unscaled=unscaled, **kwargs)

    def blink(self, value=ursina.color.clear, duration=.1, delay=0, curve=curve.in_expo_boomerang, interrupt='finish',
              unscaled=False, **kwargs):
        return self.animate_color(value, duration=duration, delay=delay, curve=curve, interrupt=interrupt,
                                  unscaled=unscaled, **kwargs)
    ...
    def intersects(self, traverse_target=scene, ignore: list = None, debug=False):
        ...
        if isinstance(self.collider, MeshCollider):
            raise Exception(
                '''error: mesh colliders can't intersect other shapes, only primitive shapes can. Mesh colliders can "receive" collisions though.''')
